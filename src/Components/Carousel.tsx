  ...
  return (
    <div style={styles.container} className="animated-carousel-container">
      ...
      {props.withNavigation && (
        <div className="animated-carousel-dots z-50" style={{ zIndex: 1 }}>
          {props?.slides.map((slide, index) => (
            <button
              key={`${index}`}
              className={`animated-carousel-dot z-50${
                index === state.active ? "active z-50" : "z-50"
              }`.trim()}
...
function getAnimationStyle({
  ...
  let transitionPostfix: string = `${(duration || 700) / 1000}s  ${
